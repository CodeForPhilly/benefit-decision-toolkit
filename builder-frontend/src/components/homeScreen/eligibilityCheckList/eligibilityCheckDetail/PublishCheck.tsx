...
const PublishCheck = ({
  ...
  const [relatedPublishedChecks] = createResource(
    () => eligibilityCheck().id, getRelatedPublishedChecks
  );
            ...
            <For each={sortedPublishedChecks()}>
              {(checkVersion) => (
                <div class="relative p-4 w-96 border-2 border-gray-200 rounded">
                  ...
                  <div>
                    <div>
                      <span class="font-bold">Module:</span> {checkVersion.module}
                    </div>
                    <div>
                      <span class="font-bold">Number of Parameters:</span> {checkVersion.parameters.length}
                    </div>
                    <div>
